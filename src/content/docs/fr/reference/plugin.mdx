...
description: Un aperçu de l'API des plugins Starlight.
sidebar:
  label: "[seti:html] Plugins"
  order: 4
---

Les plugins Starlight peuvent personnaliser la configuration, l'interface utilisateur et le comportement de Starlight, tout en étant faciles à partager et à réutiliser.
Cette page de référence documente l'API à laquelle les plugins ont accès.

Pour en savoir plus sur l'utilisation d'un plugin Starlight, consultez la [Référence de Configuration](/dockit/reference/configuration/#plugins) ou visitez la [vitrine des plugins](/dockit/resources/plugins/#plugins) pour voir une liste des plugins disponibles.

## Référence API rapide

Un plugin Starlight a la forme suivante.
Voir ci-dessous pour plus de détails sur les différentes propriétés et paramètres de hook.
...
These translations will be available via [`useTranslations()`](#usetranslations) in the `config:setup` hook and in UI components via [`Astro.locals.t()`](/dockit/guides/i18n/#using-ui-translations).
...
A callback function to add or update translation strings used in Starlight’s [localization APIs](/dockit/guides/i18n/#using-ui-translations).
...
To use the injected translations in your plugin UI, follow the [“Using UI translations” guide](/dockit/guides/i18n/#using-ui-translations).
...
A read-only copy of the user-supplied [Starlight configuration](/dockit/reference/configuration/).
...
A callback function to update the user-supplied [Starlight configuration](/dockit/reference/configuration/).
...
In the following example, a new [`social`](/dockit/reference/configuration/#social) media account is added to the existing configuration by spreading `config.social` into the new `social` array:
...
A callback function to add a [route middleware handler](/dockit/guides/route-data/) to the site.
...
To learn more about the available APIs, see the [“Using UI translations”](/dockit/guides/i18n/#using-ui-translations) guide.
